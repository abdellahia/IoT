...
IPAddress myip(192, 168, 59, 2);
byte mac[] = { 0xF8, 0xF0, 0x05, 0xEA, 0x58, 0xC3 };
...
//Wifi 
char ssid[] = "iPhone de Abdellah"; //  your network SSID (name)
...
void callback_lum(CoapPacket &packet, IPAddress ip, int port){
  ...
  Serial.println(l);
  if(l>600){  
    sprintf(response, "%d := Jour", l);
  }
  else if(l<600 && l>200){
    sprintf(response, "%d := Soir", l);
  }
  else {
    sprintf(response, "%d := Nuit", l);
  }
...
void callback_wn(CoapPacket &packet, IPAddress ip, int port){
  ...
  coap.sendResponse(ip, port, packet.messageid, "</.well-known/core>, </temp>, </hum>, </lum>");
}
